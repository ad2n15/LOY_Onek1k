...
data_matrix = pd.read_csv("data_matrix_mono_cells_for_phate.csv", index_col=0)  # Row names are treated as index
...
phate_df.to_csv("phate_result_mono.csv", index=True)  # Save with row indices (e.g., cell names)
...
plt.title("PHATE Analysis monocytes")
plt.xlabel("PHATE1")
plt.ylabel("PHATE2")
plt.savefig("phate_plot_mono.png", dpi=300)  # Save plot as PNG
plt.show()
