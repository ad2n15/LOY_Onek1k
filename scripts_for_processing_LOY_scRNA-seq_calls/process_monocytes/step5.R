...
phate_results <- read.csv("phate_result_mono.csv", row.names = 1)
...
saveRDS(male_cells, file = "male_mono_cells_191124_3.rds")
...
ggsave(filename = "UMAP_Clustering_Male_mono_Cells_surat_clusters_phate.png", plot = p1,
...
ggsave(filename = "UMAP_Clustering_Male_mono_Cells_new_classification_phate.png", plot = p1,
       width = 8, height = 6, dpi = 300)
...
sds <- slingshot(em, clusterLabels = male_cells$seurat_clusters,
                 start.clus = 0, end.clus = 2)
...
# Save the plot
ggsave("mono_cells_curve1.png", p, height = 5.5, width = 8.5)
...
saveRDS(subdata, file = "male_mono_cells_191124_4.rds")
...
# Save the plot
ggsave("PHATE_with_quantiles_mono_cells.png", p, height = 5.5, width = 8.5)

##############################
# plot using cell type

# Convert new_classification to a factor
phate_data$new_classification <- as.factor(phate_data$new_classification)
...
# Save the plot
ggsave("PHATE_with_celltype_mono_cells.png", p, height = 5.5, width = 8.5)
...
hvg_genes <- VariableFeatures(subdata)
length(hvg_genes)


# Check if any of the highly variable genes are missing from the scaled data
missing_genes <- setdiff(hvg_genes, rownames(subdata@assays$RNA@scale.data))
if (length(missing_genes) > 0) {
  warning("The following highly variable genes are missing from the scaled data: ", paste(missing_genes, collapse = ", "))
}

scaled_data <- subdata@assays$RNA@scale.data[hvg_genes, , drop = FALSE] %>%
  as.data.frame() %>%
...
fwrite(combined_results, "Combined_LOY_status_results_with_gene_names_mono_cells.csv")
...
ggsave("volcano_plot_with_top10_genes_repelled_mono_cells_linear.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
...
# Save the plot as a PNG file
ggsave("volcano_plot_with_top10_genes_repelled_mono_cells_quadratic.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
...
ggsave("classification_LOY_status_plot_mono.png", classification_plot, width = 8, height = 6, dpi = 300)
...
ggsave("quantile_LOY_status_plot_mono.png", quantile_plot, width = 8, height = 6, dpi = 300)

#############


# List of genes to analyze
genes_of_interest <- c("ENSG00000229807", "ENSG00000257764", "ENSG00000184009", "ENSG00000169385", "ENSG00000170458", "ENSG00000203747") # Replace with your gene list
...
for (gene in genes_of_interest) {
  ...
  if (gene %in% rownames(subdata@assays$RNA@counts)) {
    ...
    if (gene %in% rownames(subdata@assays$RNA@scale.data)) {
      ...
      file_name <- paste0("expression_across_quantiles_with_fit_lines_mono_", gene, ".png")
