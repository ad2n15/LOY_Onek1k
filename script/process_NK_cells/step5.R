...
male_cells <- readRDS("male_NK_cells_191124_2.rds")

# Step 1: Load the PHATE results
phate_results <- read.csv("phate_result_NK.csv", row.names = 1)
...
saveRDS(male_cells, file = "male_NK_cells_191124_3.rds")
...
ggsave(filename = "UMAP_Clustering_Male_NK_Cells_surat_clusters_phate.png", plot = p1,
...
ggsave(filename = "UMAP_Clustering_Male_NK_Cells_new_classification_phate.png", plot = p1,
       width = 8, height = 6, dpi = 300)
...
# Save the plot
ggsave("cells_curve2_NK.png", p, height = 5.5, width = 8.5)
...
saveRDS(subdata, file = "male_NK_cells_191124_4.rds")
...
# Save the plot
ggsave("PHATE_with_quantiles_NK_cells.png", p, height = 5.5, width = 8.5)
################
...
# Save the plot
ggsave("PHATE_with_celltype_NK_cells.png", p, height = 5.5, width = 8.5)
...
fwrite(combined_results, "Combined_LOY_status_results_with_gene_names_NK_cells.csv")
...
ggsave("volcano_plot_with_top10_genes_repelled_NK_cells_linear.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
...
# Save the plot as a PNG file
ggsave("volcano_plot_with_top10_genes_repelled_NK_cells_quadratic.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
...
ggsave("classification_LOY_status_plot_NK_cells.png", classification_plot, width = 8, height = 6, dpi = 300)
...
ggsave("quantile_LOY_status_plot_NK_Cells.png", quantile_plot, width = 8, height = 6, dpi = 300)
...
for (gene in genes_of_interest) {
  ...
  if (gene %in% rownames(subdata@assays$RNA@counts)) {
    ...
    if (gene %in% rownames(subdata@assays$RNA@scale.data)) {
      ...
      file_name <- paste0("expression_across_quantiles_with_fit_lines_NK_", gene, ".png")
