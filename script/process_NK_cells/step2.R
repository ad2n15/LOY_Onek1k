...
male_cells <- readRDS("male_NK_cells_191124.rds")
...
male_cells <- subset(x = male_cells, idents = c("1", "3", "5", "6", "7", "8", "9", "10"), invert = TRUE)
# Classify clusters into "B-in" and "B-mem"
male_cells$new_classification <- male_cells$seurat_clusters
male_cells$new_classification[male_cells$seurat_clusters %in% c("0", "4")] <- "NK"
...
saveRDS(male_cells, file = "male_NK_cells_191124_2.rds")
...
ggsave(filename = "UMAP_Clustering_Male_NK_Cells_newclassification.png", plot = umap_plot,
...
write.csv(dge_results, file = "DGE_results_NK_LOY_with_gene_names.csv", row.names = FALSE)
...
ggsave("volcano_plot_NK_LOY.png", volcano_plot, width = 8, height = 6)
###################################################
# Ensure 'B-in' cells are the focus
Idents(object= male_cells) <- "new_classification"  # Set cluster classification as identity
...
# Save the results to a CSV file
write.csv(dge_results, file = "DGE_results_NKr_LOY_with_gene_names.csv", row.names = FALSE)
...
# Save the volcano plot
ggsave("volcano_plot_NKr_LOY.png", volcano_plot, width = 8, height = 6)
