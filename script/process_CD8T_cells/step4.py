...
data_matrix = pd.read_csv("data_matrix_CD8T_cells_for_phate.csv", index_col=0)  # Row names are treated as index
...
phate_df.to_csv("phate_result_CD8T.csv", index=True)  # Save with row indices (e.g., cell names)
...
plt.xlabel("PHATE1")
plt.ylabel("PHATE2")
plt.savefig("phate_plot_CD8T.png", dpi=300)  # Save plot as PNG
plt.show()
