...
male_cells <- readRDS("female_CD8T_cells_191124.rds")
...
#male_cells <- subset(x = male_cells, idents = c("3", "4"), invert = TRUE)

# Classify clusters into "B-in" and "B-mem"
male_cells$new_classification <- male_cells$seurat_clusters
male_cells$new_classification[male_cells$seurat_clusters %in% c("0", "3")] <- "CD8et"
male_cells$new_classification[male_cells$seurat_clusters %in% c("1", "2")] <- "CD8nc"
...
saveRDS(male_cells, file = "female_CD8T_cells_191124_2.rds")
...
ggsave(filename = "UMAP_Clustering_FeMale_CD8T_Cells_newclassification.png", plot = umap_plot,
...
male_cells_B_in <- subset(male_cells, idents = "CD8nc")  # Subset only 'B-in' cells
male_cells_B_in
...
write.csv(dge_results, file = "DGE_results_CD8nc_in_LOY_with_gene_names_female.csv", row.names = FALSE)
...
ggsave("volcano_plot_CD8nc_LOY_female.png", volcano_plot, width = 8, height = 6)
###################################################
# Ensure 'B-in' cells are the focus
Idents(object= male_cells) <- "new_classification"  # Set cluster classification as identity
male_cells_B_mem <- subset(male_cells, idents = "CD8et")  # Subset only 'B-in' cells
male_cells_B_mem
...
# Save the results to a CSV file
write.csv(dge_results, file = "DGE_results_CD8et_mem_LOY_with_gene_names_female.csv", row.names = FALSE)
...
# Save the volcano plot
ggsave("volcano_plot_CD8et_LOY_female.png", volcano_plot, width = 8, height = 6)
