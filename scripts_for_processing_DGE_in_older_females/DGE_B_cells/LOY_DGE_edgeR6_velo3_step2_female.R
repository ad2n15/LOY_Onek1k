...
male_cells <- readRDS("female_B_cells_191124.rds")
...
male_cells <- subset(x = male_cells, idents = c("4", "6", "8", "9", "10", "11", "13"), invert = TRUE)
...
male_cells$new_classification[male_cells$seurat_clusters %in% c("0", "1", "5", "7", "12")] <- "B-in"
male_cells$new_classification[male_cells$seurat_clusters %in% c("2", "3")] <- "B-mem"
...
saveRDS(male_cells, file = "female_B_cells_191124_2.rds")
...
ggsave(filename = "UMAP_Clustering_FeMale_B_Cells_newclassification.png", plot = umap_plot,
...
write.csv(dge_results, file = "DGE_results_B_in_age_with_gene_names_female.csv", row.names = FALSE)
...
ggsave("volcano_plot_B_in_age_female.png", volcano_plot, width = 8, height = 6)
###################################################
# Ensure 'B-in' cells are the focus
Idents(object= male_cells) <- "new_classification"  # Set cluster classification as identity
...
# Save the results to a CSV file
write.csv(dge_results, file = "DGE_results_B_mem_age_with_gene_names_female.csv", row.names = FALSE)
...
# Save the volcano plot
ggsave("volcano_plot_B_mem_age_female.png", volcano_plot, width = 8, height = 6)
